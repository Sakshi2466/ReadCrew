...
// ─── Helper: get stored profile image for any user email ─────────────────
      ...
      if (res.ok) {
        ...
        if (links) {
          ...
          if (raw) {
            ...
            setCoverUrl(clean); setIsLoading(false); return;
          }
        }
      }
    } catch {}
    try {
      const res = await fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=1`, { signal: AbortSignal.timeout(5000) });
      if (res.ok) {
        const data = await res.json();
...
// ─── PUBLIC USER AVATAR (shows real photo if available) ─────────────────────
const UserAvatar = ({ email, name, size = 40, ring = false, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      style={{
        width: sizePx, height: sizePx, borderRadius: '50%', overflow: 'hidden', flexShrink: 0,
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4" style={{ maxWidth: '448px', left: '50%', transform: 'translateX(-50%)', width: '100%' }}>
      <div className="bg-white rounded-2xl w-full max-w-sm mx-auto">
        ...
        <div className="p-5">
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-orange-400 resize-none mb-3" placeholder="Add a comment..." rows={3} />
...
const PostOptionsModal = ({ post, user, onClose, onReshare, onSave, isSaved, onDelete, isOwner, onFollow, isFollowing, onBlock, isBlocked }) => {
  ...
  if (!isOwner) {
    options.push({ id: 'follow', icon: isFollowing ? UserMinus : UserPlus, label: isFollowing ? 'Unfollow' : 'Follow', color: isFollowing ? 'text-red-500' : 'text-green-600', action: () => onFollow(post.userEmail, post.userName) });
  }
  if (isOwner) {
    options.push({ id: 'delete', icon: Trash2, label: 'Delete', color: 'text-red-500', action: () => onDelete(post) });
  } else {
    options.push({ id: 'block', icon: isBlocked ? UserCheck : UserMinus, label: isBlocked ? 'Unblock' : 'Block User', color: isBlocked ? 'text-green-600' : 'text-red-500', action: () => onBlock(post.userEmail, post.userName) });
  }
  return (
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4" style={{ maxWidth: '448px', left: '50%', transform: 'translateX(-50%)', width: '100%' }}>
      <div className="bg-white rounded-2xl w-full max-w-sm mx-auto overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {options.map(option => (
            <button key={option.id} onClick={() => { option.action(); onClose(); }} className="w-full px-4 py-3 flex items-center gap-3 hover:bg-gray-50 transition">
              <option.icon className={`w-5 h-5 ${option.color}`} />
              <span className={`text-sm font-medium ${option.color}`}>{option.label}</span>
            </button>
          ))}
          <button onClick={onClose} className="w-full px-4 py-3 text-sm text-gray-500 hover:bg-gray-50 transition">Cancel</button>
        </div>
      </div>
    </div>
  );
...
// ─── INLINE POST CARD ─────────────────────────────────────────────────────
// Profile photos are PUBLIC - shown for every post author and commenter
const InlinePostCard = ({ post, user, profileSrc, updateNotificationCount, onShare, onReshareClick, onSaveToggle, isSaved, onDelete, onFollow, isFollowing, onBlock, isBlocked, onViewUserProfile }) => {
  ...
  const handleLikePost = () => {
    ...
    localStorage.setItem('allPosts', JSON.stringify(allPosts.map(p => p.id === post.id ? { ...p, likes: (p.likes || 0) + 1 } : p)));
    if (post.userEmail !== user.email) {
      const notif = { id: Date.now(), type: 'like', fromUser: user.name, fromUserEmail: user.email, message: `${user.name} liked your post`, timestamp: new Date().toISOString(), read: false, postId: post.id };
  ...
  const handlePostComment = () => {
    ...
    const mentions = [];
    let match;
    while ((match = mentionRegex.exec(newComment)) !== null) mentions.push(match[1]);
    const comment = { id: Date.now(), postId: post.id, userId: user.id, userName: user.name, userEmail: user.email, userInitials: user.name.slice(0, 2).toUpperCase(), content: newComment.trim(), timestamp: new Date().toISOString(), parentId: replyTo?.id || null, likes: 0, mentions };
    ...
    if (post.userEmail !== user.email) {
      const notif = { id: Date.now(), type: 'comment', fromUser: user.name, fromUserEmail: user.email, message: `${user.name} commented: "${newComment.substring(0, 40)}"`, timestamp: new Date().toISOString(), read: false, postId: post.id };
      const notifs = JSON.parse(localStorage.getItem(`user_${post.userEmail}_notifications`) || '[]');
      notifs.unshift(notif);
      localStorage.setItem(`user_${post.userEmail}_notifications`, JSON.stringify(notifs));
      window.dispatchEvent(new StorageEvent('storage', { key: `user_${post.userEmail}_notifications` }));
  ...
  // CommentRow with PUBLIC profile photo
      ...
      <div className={`flex gap-3 ${isReply ? 'mt-3' : ''}`}>
        ...
        <div className="flex-1 min-w-0 pb-3">
          ...
          <div className="flex items-center gap-4 mt-1.5">
            <button onClick={() => handleLikeComment(comment.id)} disabled={liked} className={`flex items-center gap-1.5 text-xs font-medium transition-all ${liked ? 'text-red-500' : 'text-gray-400 hover:text-red-400'}`}>
              <Heart className={`w-3.5 h-3.5 ${liked ? 'fill-red-500' : ''}`} /><span>{comment.likes || 0}</span>
            </button>
            <button onClick={() => { setReplyTo(comment); setTimeout(() => inputRef.current?.focus(), 100); }} className="flex items-center gap-1 text-xs text-gray-400 hover:text-orange-500 font-semibold">
              <Share2 className="w-3 h-3 rotate-180" />Reply
            </button>
          ...
          {replies.length > 0 && (
            <div className="mt-2">
              {!showReplies[comment.id] && (
                <button onClick={() => setShowReplies(p => ({ ...p, [comment.id]: true }))} className="text-xs text-orange-500 font-semibold mb-2">↳ View {replies.length} {replies.length === 1 ? 'reply' : 'replies'}</button>
              )}
              {showReplies[comment.id] && (
  ...
  return (
    <>
      {showOptions && (
        <PostOptionsModal post={post} user={user} onClose={() => setShowOptions(false)} onReshare={onReshareClick} onSave={onSaveToggle} isSaved={isSaved} onDelete={onDelete} isOwner={isPostAuthor} onFollow={onFollow} isFollowing={isFollowing} onBlock={onBlock} isBlocked={isBlocked} />
      )}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-4 pt-4 pb-3">
          <div className="flex items-start gap-3">
            {/* PUBLIC profile photo for post author */}
            <UserAvatar
        ...
        <div className="px-4 pb-3">
          ...
          {post.isReshare && post.originalPost && (
            <div className="flex items-center gap-1 text-xs text-gray-500 mb-2"><Repeat className="w-3 h-3" /><span>Reshared from <span className="font-semibold">{post.originalPost.userName}</span></span></div>
          )}
        ...
        <div className="px-4 py-2.5 border-t border-gray-100 flex items-center gap-5">
          ...
          <button onClick={() => inputRef.current?.focus()} className="flex items-center gap-1.5 text-sm font-semibold text-gray-500 hover:text-orange-500 transition">
        ...
        <div className="px-4 py-3 border-t border-gray-50 bg-gray-50/60">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button onClick={handlePostComment} disabled={!newComment.trim()} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${newComment.trim() ? 'bg-orange-500 text-white shadow-sm active:scale-95' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>Post</button>
          </div>
        </div>
        ...
        {comments.length > 0 && (
          <div className="px-4 py-3 border-t border-gray-100 space-y-1">
            ...
            {topLevel.length > 3 && (
              <button onClick={() => setShowAllComments(p => !p)} className="text-xs text-orange-500 font-semibold mt-1 flex items-center gap-1 hover:text-orange-600">
...
const LoginPage = ({ onLogin }) => {
  ...
  const saveLocalUser = (userData) => {
    ...
    ['followers','following','blocked'].forEach(key => {
      if (!localStorage.getItem(`user_${userData.email}_${key}`)) localStorage.setItem(`user_${userData.email}_${key}`, JSON.stringify([]));
    });
    if (!localStorage.getItem(`user_${userData.email}_stats`)) localStorage.setItem(`user_${userData.email}_stats`, JSON.stringify(userData.stats || { booksRead: 0, reviewsGiven: 0, postsCreated: 0, crewsJoined: 0 }));
    return userData;
  };

  const handleSendOTP = async () => {
    setErrorMsg('');
    if (!name.trim() || name.trim().length < 2) { setErrorMsg('Please enter your full name'); return; }
    if (!validateEmail(email)) { setErrorMsg('Please enter a valid email'); return; }
    setLoading(true);
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    localStorage.setItem('devOTP', otp);
    setShowOTP(true); setDevOtpDisplay(otp);
    setInfoMsg('Email service unavailable — use code below:');
    setLoading(false);
  };
  ...
  const handleVerifyOTP = () => {
    ...
    if (otpInput !== devOTP) { setErrorMsg('❌ Incorrect code'); return; }
  ...
  const handleLogin = () => {
    setErrorMsg('');
    if (!validateEmail(email)) { setErrorMsg('Enter a valid email'); return; }
    if (!password.trim()) { setErrorMsg('Enter your password'); return; }
    const users = JSON.parse(localStorage.getItem('users') || '[]');
  ...
  if (showOTP) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-7 border border-gray-200">
        ...
        {devOtpDisplay && <div className="bg-amber-50 border border-amber-300 rounded-2xl p-4 mb-4 text-center"><p className="text-xs text-amber-700 font-medium mb-2">📧 Use this code:</p><p className="text-4xl font-bold text-amber-800 tracking-widest">{devOtpDisplay}</p></div>}
        ...
        <button onClick={() => { setShowOTP(false); setErrorMsg(''); }} className="text-gray-500 text-sm flex items-center gap-1"><ArrowLeft className="w-4 h-4" />Back</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-7 border border-gray-200">
          ...
          <button onClick={isLogin ? handleLogin : handleSendOTP} disabled={loading} className="w-full mt-5 py-4 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-2xl font-semibold disabled:opacity-60">{loading ? 'Please wait...' : isLogin ? 'Log In' : 'Create Account →'}</button>
          <p className="text-center text-sm text-gray-500 mt-4">
            {isLogin ? "Don't have an account? " : "Already have an account? "}
            <button onClick={() => { setIsLogin(!isLogin); setErrorMsg(''); }} className="text-orange-500 font-semibold">{isLogin ? 'Sign Up' : 'Log In'}</button>
          </p>
        </div>
      </div>
    </div>
...
const BOOK_DB = {
  thriller: [
    { title: 'Gone Girl', author: 'Gillian Flynn', genre: 'Thriller', description: 'A woman vanishes on her anniversary.', reason: 'Twisty, addictive, impossible to put down', rating: 4.6, pages: 422, year: 2012 },
    { title: 'The Silent Patient', author: 'Alex Michaelides', genre: 'Thriller', description: 'A famous painter shoots her husband and never speaks again.', reason: 'Jaw-dropping twist', rating: 4.5, pages: 336, year: 2019 },
    { title: 'Verity', author: 'Colleen Hoover', genre: 'Thriller', description: 'A writer discovers a disturbing manuscript.', reason: 'You will NOT see the ending coming', rating: 4.6, pages: 336, year: 2018 },
  ],
  fantasy: [
    { title: 'The Name of the Wind', author: 'Patrick Rothfuss', genre: 'Fantasy', description: 'Kvothe tells his own extraordinary story.', reason: 'Stunning prose and world-building', rating: 4.7, pages: 662, year: 2007 },
    { title: 'Mistborn: The Final Empire', author: 'Brandon Sanderson', genre: 'Fantasy', description: 'A crew of thieves plots to rob an immortal god-emperor.', reason: 'Inventive magic system', rating: 4.7, pages: 541, year: 2006 },
    { title: 'Fourth Wing', author: 'Rebecca Yarros', genre: 'Fantasy', description: 'A war college for dragon riders.', reason: 'Fast-paced and addictive', rating: 4.6, pages: 528, year: 2023 },
  ],
  romance: [
    { title: 'Beach Read', author: 'Emily Henry', genre: 'Romance', description: 'Two rival authors swap genres and fall in love.', reason: 'Witty and heartfelt', rating: 4.6, pages: 361, year: 2020 },
    { title: 'It Ends with Us', author: 'Colleen Hoover', genre: 'Romance', description: 'A powerful story about love and resilience.', reason: 'Emotional and beautifully written', rating: 4.6, pages: 368, year: 2016 },
  ],
  scifi: [
    { title: 'Project Hail Mary', author: 'Andy Weir', genre: 'Sci-Fi', description: 'A lone astronaut must save Earth from extinction.', reason: 'Most fun sci-fi you will read', rating: 4.8, pages: 476, year: 2021 },
    { title: 'Dune', author: 'Frank Herbert', genre: 'Sci-Fi', description: 'A young nobleman navigates politics on a desert planet.', reason: 'Foundation of modern science fiction', rating: 4.8, pages: 688, year: 1965 },
  ],
  selfhelp: [
    { title: 'Atomic Habits', author: 'James Clear', genre: 'Self-Help', description: 'Tiny changes lead to remarkable results.', reason: 'Most practical habit book ever written', rating: 4.8, pages: 320, year: 2018 },
    { title: 'The Psychology of Money', author: 'Morgan Housel', genre: 'Finance', description: 'Timeless lessons on wealth from 19 short stories.', reason: 'Changes how you think about money forever', rating: 4.7, pages: 256, year: 2020 },
  ],
  literary: [
    { title: 'The Midnight Library', author: 'Matt Haig', genre: 'Fiction', description: 'A library between life and death.', reason: 'Beautiful and profoundly hopeful', rating: 4.6, pages: 288, year: 2020 },
    { title: 'The Alchemist', author: 'Paulo Coelho', genre: 'Inspirational', description: 'A shepherd boy pursues his personal legend.', reason: 'Short, profound and endlessly re-readable', rating: 4.7, pages: 197, year: 1988 },
  ],
};
...
const generateClientResponse = (userText, previousBooks = []) => {
  ...
  const detectCategory = () => {
    if (/thrille|suspens|crime|murder|dark/i.test(text)) return 'thriller';
    if (/fantasy|magic|dragon|wizard/i.test(text)) return 'fantasy';
    if (/romance|love|swoony/i.test(text)) return 'romance';
    if (/sci.?fi|space|future|robot/i.test(text)) return 'scifi';
    if (/self.?help|habit|motivat|improve/i.test(text)) return 'selfhelp';
    return 'literary';
  };
  ...
  const intros = { thriller: "5 gripping thrillers you won't put down! 🔪", fantasy: "5 magical worlds waiting for you ✨", romance: "5 romance reads that'll give you all the feels ❤️", scifi: "5 sci-fi journeys to blow your mind 🚀", selfhelp: "5 books that'll genuinely change how you think 💡", literary: "5 beautifully written books that'll stay with you 📚" };
  return { reply: intros[category], books: recs };
};
...
const BookCard = ({ book, onCreateCrew }) => (
  <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
    ...
    <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
      ...
      <button onClick={() => navigator.clipboard.writeText(`"${book.title}" by ${book.author}`)} className="px-4 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 transition"><Share2 className="w-4 h-4 text-gray-500" /></button>
    </div>
  </div>
);

// ─── USER PROFILE MODAL ────────────────────────────────────────────────────
// Books read and saved posts are PRIVATE — not shown for other users
const UserProfileModal = ({ userEmail, userName, currentUser, onClose, onFollow, isFollowing, onViewFullProfile, onBlock, isBlocked }) => {
  ...
  useEffect(() => {
    ...
    setUserReviews(allReviews.filter(r => r.userEmail === userEmail).slice(0, 3));
  }, [userEmail]);
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm mx-auto max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5">
          ...
          <div className="bg-gray-50 rounded-xl p-3 mb-4 flex items-center gap-2">
            ...
            <p className="text-xs text-gray-500">Books read and saved posts are private</p>
          </div>
...
// Books Read and Saved Posts are PRIVATE — only Posts, Reviews, Crews are visible
const FullUserProfilePage = ({ viewedUserEmail, viewedUserName, currentUser, onBack, onFollow, isFollowing, onBlock, isBlocked }) => {
  ...
  useEffect(() => {
    ...
    setUserCrews(allCrews.filter(c => joinedCrews.includes(c.id) || joinedCrews.includes(String(c.id))));
  }, [viewedUserEmail]);

  // PUBLIC tabs: Posts, Reviews, Crews
  // PRIVATE tabs (only shown for own profile): Books Read — handled in ProfilePage
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 overflow-y-auto">
      ...
      <div className="px-4 py-5">
        ...
        {/* Privacy notice for others viewing this profile */}
        {!isOwnProfile && (
        ...
        <div className="flex border-b border-gray-200 mb-4">
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 text-sm pb-2.5 px-3 font-medium border-b-2 transition ${activeTab === tab ? 'text-orange-500 border-orange-500' : 'text-gray-500 border-transparent'}`}>{tab}</button>
          ))}
        </div>

        {activeTab === 'Posts' && (
          <div className="space-y-4">
            {userPosts.length === 0 ? <div className="text-center py-8"><Edit3 className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No posts yet</p></div>
              : userPosts.map(post => (
        ...
        {activeTab === 'Reviews' && (
          <div className="space-y-4">
            {userReviews.length === 0 ? <div className="text-center py-8"><Star className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No reviews yet</p></div>
              : userReviews.map(review => (
                <div key={review.id} className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <div className="flex-1"><h3 className="font-semibold text-sm">{review.bookName}</h3><p className="text-xs text-gray-500">by {review.author}</p><StarRating rating={review.rating} size="xs" /></div>
                  </div>
        ...
        {activeTab === 'Crews' && (
          <div className="space-y-3">
            {userCrews.length === 0 ? <div className="text-center py-8"><Users className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No crews joined yet</p></div>
              : userCrews.map(crew => (
...
// ─── HOME PAGE ──────────────────────────────────────────────────────────────
const HomePage = ({ user, posts, setPosts, crews, setPage, donations, reviews, onUpdateStats, updateNotificationCount, profileSrc, savedPosts, onSavePost, onResharePost, onDeletePost, onFollow, following, onBlock, blockedUsers, onViewUserProfile }) => {
...
const ExplorePage = ({ user, setPage, onCreateCrew }) => {
  const [chatMessages, setChatMessages] = useState([
    { role: 'assistant', content: "Hey! 👋 I'm Page Turner, your AI book guide. Tell me what you're in the mood for — a genre, a vibe, or the last book you loved!", timestamp: new Date() }
  ]);
      ...
      <div className="mx-4 space-y-3">
        ...
        {loading && (
          <div className="flex justify-start items-end gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-orange-500 to-red-500 rounded-full flex items-center justify-center flex-shrink-0"><BookOpen className="w-4 h-4 text-white" /></div>
...
const ReviewsPage = ({ user, setPage, updateNotificationCount, onViewUserProfile }) => {
  ...
  const filteredReviews = reviews.filter(r => r.bookName.toLowerCase().includes(searchQuery.toLowerCase()) || r.author.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="pb-24 bg-gray-50 min-h-screen overflow-y-auto">
      ...
      <div className="px-4 py-4">
        <div className="mb-4 relative">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by book or author..." className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-orange-400" />
        ...
        {showCreateForm && (
          <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm mb-4">
            ...
            <div className="space-y-3 mb-4">
              <input type="text" value={newReview.bookName} onChange={e => setNewReview({...newReview, bookName: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Book name" />
              <input type="text" value={newReview.author} onChange={e => setNewReview({...newReview, author: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Author" />
              ...
              <textarea value={newReview.review} onChange={e => setNewReview({...newReview, review: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none resize-none" placeholder="Write your review..." rows={4} />
              <div className="flex gap-2">
                {['positive','negative'].map(s => <button key={s} type="button" onClick={() => setNewReview({...newReview, sentiment: s})} className={`flex-1 py-2 rounded-lg text-sm font-medium ${newReview.sentiment === s ? (s === 'positive' ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : 'bg-gray-100 text-gray-600'}`}>{s === 'positive' ? '👍 Positive' : '👎 Negative'}</button>)}
              </div>
            </div>
            ...
            {filteredReviews.map((review, idx) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                  <div className="flex items-start gap-3 mb-2">
                    <DynamicBookCover title={review.bookName} author={review.author} size="sm" />
                    <div className="flex-1"><h3 className="font-semibold text-gray-900 text-sm">{review.bookName}</h3><p className="text-xs text-gray-500">by {review.author}</p><StarRating rating={review.rating} size="xs" /></div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={() => handleLikeReview(review.id, review)} disabled={isLiked} className={`flex items-center gap-1 text-xs ${isLiked ? 'text-red-500' : 'text-gray-400 hover:text-red-400'}`}><Heart className={`w-3.5 h-3.5 ${isLiked ? 'fill-red-500' : ''}`} />{review.likes||0}</button>
...
// ─── CREWS PAGE ─────────────────────────────────────────────────────────────
// - One book, one author, one policy per crew (enforced uniqueness)
// - ALL crews in discover with search
// - Orange own messages in chat
// - Real profile photos visible in chat
const CrewsPage = ({ user, crews: initialCrews, setPage, updateNotificationCount, onViewUserProfile }) => {
  ...
  const [newCrewData, setNewCrewData] = useState({ name: '', author: '', genre: '', policy: '' });
  ...
  useEffect(() => {
    const raw = JSON.parse(localStorage.getItem('crews') || '[]');
    // Deduplicate: keep only the first crew per unique book+author
    const seen = new Set();
    const deduped = (raw.length > 0 ? raw : initialCrews).filter(c => {
      const key = `${c.name.trim().toLowerCase()}__${c.author.trim().toLowerCase()}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
    if (deduped.length !== raw.length) {
      // Persist the cleaned list back to storage
      localStorage.setItem('crews', JSON.stringify(deduped));
    }
    setCrews(deduped);
  ...
  useEffect(() => {
    if (selectedCrew) {
      ...
      const members = allUsers.filter(u => {
        const jc = JSON.parse(localStorage.getItem(`user_${u.email}_joinedCrews`) || '[]');
        return jc.includes(selectedCrew.id) || jc.includes(String(selectedCrew.id));
      }).map(u => ({ id: u.id, name: u.name, email: u.email, initials: u.name?.slice(0,2), online: Math.random() > 0.5 }));
      if (!members.find(m => m.email === selectedCrew.createdBy)) {
        members.push({ id: selectedCrew.createdBy, name: selectedCrew.createdByName||'Creator', email: selectedCrew.createdBy, initials: (selectedCrew.createdByName||'CR').slice(0,2), online: true, isCreator: true });
      }
  ...
  // Uniqueness validation: one book+author, one policy across all crews
  const validateNewCrew = (data) => {
    const bookAuthorTaken = crews.some(c =>
      c.name.toLowerCase() === data.name.trim().toLowerCase() &&
      c.author.toLowerCase() === data.author.trim().toLowerCase()
    );
    if (bookAuthorTaken) return 'A crew for this book by this author already exists!';

    if (data.policy.trim()) {
      const policyTaken = crews.some(c =>
        c.policy && c.policy.toLowerCase() === data.policy.trim().toLowerCase()
      );
      if (policyTaken) return 'Another crew already has this exact policy. Please write a unique policy.';
    }
    return null;
  };
  ...
  const createCrew = () => {
    setCreateError('');
    if (!newCrewData.name || !newCrewData.author) { setCreateError('Book title and author are required'); return; }
    const err = validateNewCrew(newCrewData);
    if (err) { setCreateError(err); return; }

    const newCrew = {
      id: Date.now(),
      name: newCrewData.name.trim(),
      author: newCrewData.author.trim(),
      genre: newCrewData.genre.trim(),
      policy: newCrewData.policy.trim(),
      members: 1, chats: 0,
    ...
    setNewCrewData({ name: '', author: '', genre: '', policy: '' });
  };
  ...
  const formatTime = (ts) => {
    const diff = Date.now() - new Date(ts);
    const mins = Math.floor(diff/60000), hrs = Math.floor(diff/3600000), days = Math.floor(diff/86400000);
    if (mins < 1) return 'Just now'; if (mins < 60) return `${mins}m`; if (hrs < 24) return `${hrs}h`; if (days < 7) return `${days}d`;
    return new Date(ts).toLocaleDateString();
  };
  ...
  // ALL crews shown in discover (search filters both joined and not-joined)
  const filteredCrews = crews.filter(crew =>
    ...
    (crew.policy && crew.policy.toLowerCase().includes(searchQuery.toLowerCase()))
  );
    ...
    return (
      <div className="fixed inset-0 flex flex-col z-[60] overflow-hidden" style={{ maxWidth: '448px', left: '50%', transform: 'translateX(-50%)', width: '100%', background: '#ece5dd' }}>
        ...
        <div className="flex-shrink-0 bg-[#C8622A] px-4 py-2.5 flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3">
            <button onClick={() => setView('detail')} className="p-1 rounded-full hover:bg-white/20"><ChevronLeft className="w-5 h-5 text-white" /></button>
          ...
          <button className="p-2 rounded-full hover:bg-white/20"><MoreHorizontal className="w-5 h-5 text-white" /></button>
        </div>

        {/* Policy banner if crew has one */}
        {selectedCrew.policy && (
          <div className="bg-orange-50 border-b border-orange-200 px-4 py-2 flex items-center gap-2">
            <Shield className="w-3.5 h-3.5 text-orange-500 flex-shrink-0" />
            <p className="text-xs text-orange-700 font-medium">Policy: {selectedCrew.policy}</p>
          </div>
        )}
                    ...
                    return (
                      <div key={msg.id} className={`flex mb-2 ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex max-w-[80%] items-end gap-2 ${isOwn ? 'flex-row-reverse' : ''}`}>
                          {/* PUBLIC profile photo visible in crew chat */}
                          {!isOwn && (
                            <UserAvatar
                              email={msg.userEmail}
                              name={msg.userName}
                          ...
                          <div className={`rounded-2xl px-3.5 py-2 shadow-sm ${
                            isOwn
                              ? 'bg-[#C8622A] rounded-br-sm'   // ← ORANGE for own messages
                              : 'bg-white rounded-bl-sm'
                          }`}>
                            {!isOwn && <p className="text-xs font-bold text-[#C8622A] mb-0.5">{msg.userName}</p>}
                            {msg.type === 'image'
                              ? <img src={msg.content} alt="Shared" className="max-w-full rounded-xl max-h-60" />
                              : <p className={`text-sm leading-relaxed break-words ${isOwn ? 'text-white' : 'text-gray-900'}`}>{msg.content}</p>
                            }
                            <p className={`text-[10px] text-right mt-0.5 ${isOwn ? 'text-orange-200' : 'text-gray-400'}`}>
                              {formatTime(msg.timestamp)}{isOwn && <span className="ml-1">✓✓</span>}
                            </p>
                          </div>
                        </div>
                      </div>
        ...
        {hasJoined && (
          <div className="flex-shrink-0 bg-[#ece5dd] px-3 py-2.5" style={{ paddingBottom: 'max(10px, env(safe-area-inset-bottom))' }}>
            <div className="flex items-center gap-2 bg-white rounded-full px-3 py-1.5 shadow border border-gray-100">
              <button onClick={() => fileInputRef.current?.click()} className="w-8 h-8 flex items-center justify-center flex-shrink-0"><Plus className="w-5 h-5 text-[#C8622A]" /></button>
              ...
              <button onClick={sendMessage} disabled={!newMessage.trim()} className={`w-8 h-8 flex items-center justify-center rounded-full transition ${newMessage.trim() ? 'bg-[#C8622A] text-white' : 'bg-gray-200 text-gray-400'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="bg-gradient-to-b from-orange-50 to-white px-4 pt-6 pb-4">
            <div className="flex flex-col items-center text-center">
              ...
              {selectedCrew.policy && (
                <div className="mt-3 flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-4 py-2.5 w-full">
                  <Shield className="w-4 h-4 text-orange-500 flex-shrink-0" />
                  <div className="text-left">
                    <p className="text-xs text-orange-500 font-semibold uppercase tracking-wide">Crew Policy</p>
                    <p className="text-sm text-orange-800">{selectedCrew.policy}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-3 mt-5 w-full">
                {!hasJoined ? (
                  ...
                  <button onClick={() => setView('chat')} className="flex-1 py-3 bg-[#C8622A] text-white rounded-xl font-semibold">Open Chat</button>
                )}
                <button onClick={() => { const e = prompt("Friend's email:"); if (e) { const n = { id: Date.now(), type: 'invite', fromUser: user.name, message: `${user.name} invited you to join "${selectedCrew.name}"!`, timestamp: new Date().toISOString(), read: false }; const ns = JSON.parse(localStorage.getItem(`user_${e}_notifications`) || '[]'); ns.unshift(n); localStorage.setItem(`user_${e}_notifications`, JSON.stringify(ns)); alert(`Invited ${e}!`); }}} className="px-4 py-3 border border-gray-200 rounded-xl"><UserPlus className="w-5 h-5" /></button>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 pb-24">
            {selectedTab === 'chat' && (
              hasJoined ? (
                <div className="space-y-3">
                  <button onClick={() => setView('chat')} className="w-full py-4 bg-gradient-to-r from-orange-500 to-[#C8622A] text-white rounded-xl font-semibold flex items-center justify-center gap-2"><MessageCircle className="w-5 h-5" />Open Chat</button>
                  ...
                  <div key={member.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        {/* Public profile photo in members list */}
                        <UserAvatar email={member.email} name={member.name} size={40} onClick={() => onViewUserProfile(member.email, member.name)} />
            ...
            {selectedTab === 'about' && (
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-sm text-gray-600">A crew for readers of "{selectedCrew.name}" by {selectedCrew.author}.</p>
                </div>
                {selectedCrew.policy && (
                  <div className="bg-orange-50 rounded-xl p-4 border border-orange-100">
                    <div className="flex items-center gap-2 mb-1"><Shield className="w-4 h-4 text-orange-500" /><h3 className="font-semibold text-orange-800">Crew Policy</h3></div>
                    <p className="text-sm text-orange-700">{selectedCrew.policy}</p>
                  </div>
                )}
  ...
  return (
    <div className="pb-24 bg-gray-50 min-h-screen overflow-y-auto">
      ...
      <div className="px-4 py-4">
        {/* Search — searches ALL crews */}
        <div className="mb-4 relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search all crews by book, author, genre, or policy..." className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:border-orange-400" />
          {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 transform -translate-y-1/2"><X className="w-4 h-4 text-gray-400" /></button>}
        </div>

        {/* If search returns nothing, offer to create */}
        {searchQuery && filteredCrews.length === 0 && (
          <div className="bg-white rounded-xl p-6 text-center border border-orange-200 mb-4">
            ...
            <button onClick={() => { setNewCrewData({ name: searchQuery, author: '', genre: '', policy: '' }); setShowCreateCrewForm(true); }} className="px-5 py-2.5 bg-orange-500 text-white rounded-xl text-sm font-semibold">Create this Crew</button>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl p-4 border border-orange-200 shadow-sm mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1 bg-orange-50 px-2 py-1 rounded-lg">
                ...
                <span className="text-xs text-orange-600 font-medium">One book · One author · One policy</span>
              </div>
            </div>
            {newCrewData.name && (
            ...
            {createError && <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2 mb-3 text-sm text-red-600">⚠ {createError}</div>}
            <div className="space-y-3">
              <input value={newCrewData.name} onChange={e => { setNewCrewData({...newCrewData, name: e.target.value}); setCreateError(''); }} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-300" placeholder="📖 Book title *" />
              <input value={newCrewData.author} onChange={e => { setNewCrewData({...newCrewData, author: e.target.value}); setCreateError(''); }} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-300" placeholder="✍️ Author *" />
              <input value={newCrewData.genre} onChange={e => setNewCrewData({...newCrewData, genre: e.target.value})} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-300" placeholder="🏷️ Genre (e.g. Fiction, Self-Help)" />
              <div>
                <input value={newCrewData.policy} onChange={e => { setNewCrewData({...newCrewData, policy: e.target.value}); setCreateError(''); }} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:border-orange-300" placeholder="🛡️ Crew policy (e.g. No spoilers past chapter 10)" />
                <p className="text-xs text-gray-400 mt-1">Policies must be unique across all crews</p>
              </div>
              <div className="flex gap-2">
                <button onClick={createCrew} className="flex-1 py-2.5 bg-orange-500 text-white rounded-lg text-sm font-bold">Create Crew</button>
                <button onClick={() => { setShowCreateCrewForm(false); setCreateError(''); }} className="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {!searchQuery && (
          <div className="mb-6">
            ...
            {crews.filter(c => isJoined(c.id)).length === 0 ? (
              <div className="bg-white rounded-xl p-6 text-center border border-gray-200"><p className="text-gray-500 text-sm">No crews joined yet. Explore below!</p></div>
              ...
              <div key={crew.id} className="bg-white rounded-xl overflow-hidden border border-green-200 shadow-sm cursor-pointer mb-3" onClick={() => { setSelectedCrew(crew); setView('detail'); }}>
                <div className="flex items-center px-4 gap-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">by {crew.author}</p>
                    {crew.policy && <p className="text-xs text-orange-600 mt-0.5 flex items-center gap-1"><Shield className="w-3 h-3" />{crew.policy}</p>}
                ...
                <div className="px-4 py-2 flex justify-end gap-2 border-t border-gray-100">
                  <button onClick={e => { e.stopPropagation(); setSelectedCrew(crew); setView('chat'); }} className="px-3 py-1 bg-[#C8622A] text-white rounded-lg text-xs font-medium">Chat</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          {!searchQuery && <p className="text-xs text-gray-400 mb-3">All reading crews — join any that interests you</p>}
          <div className="space-y-3">
            {filteredCrews.filter(c => !isJoined(c.id)).length === 0 && searchQuery ? null : (
...
const ProfilePage = ({ user, posts, setPage, onLogout, onUpdateUser, profileSrc, setProfileSrc, savedPosts, following, followers, onFollow }) => {
  ...
  const handleSaveGoal = () => {
    ...
    setReadingGoal(editGoal); setShowEditGoal(false);
  ...
  // All tabs available on OWN profile including private ones
      ...
      <div className="px-4 py-5">
        <div className="flex items-start gap-4 mb-5">
          <div className="relative flex-shrink-0">
            {profileSrc ? <img src={profileSrc} alt={user?.name} className="w-20 h-20 rounded-full object-cover border-2 border-orange-300 shadow-md" />
          ...
          <div className="flex-1 min-w-0">
            {editingProfile ? (
              <div className="space-y-2">
                <input value={editName} onChange={e => setEditName(e.target.value)} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Your name" />
                <input value={editBio} onChange={e => setEditBio(e.target.value)} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Your bio..." />
                <div className="flex gap-2">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-4 flex items-center gap-2">
          <Lock className="w-4 h-4 text-gray-400 flex-shrink-0" />
          <p className="text-xs text-gray-500">Your <strong>Books Read</strong> and <strong>Saved Posts</strong> are private — only you can see them</p>
        </div>
        ...
        <div className="flex border-b border-gray-200 mb-4 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 text-sm pb-2.5 px-3 font-medium border-b-2 transition ${activeTab === tab ? 'text-orange-500 border-orange-500' : 'text-gray-500 border-transparent'}`}>
              {(tab === 'Books Read' || tab === 'Saved Posts') && <Lock className="w-3 h-3 inline mr-1 opacity-50" />}{tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'Posts' && (
          <div className="space-y-4">
            {myPosts.length === 0 ? <div className="text-center py-8"><Edit3 className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No posts yet</p><button onClick={() => setPage('post')} className="mt-3 px-4 py-2 bg-orange-500 text-white rounded-lg text-sm">Create First Post</button></div>
              : myPosts.map(post => (
                <div key={post.id} className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                  <p className="text-sm text-gray-700 mb-2">{post.content}</p>
                  {post.bookName && <p className="text-xs text-orange-500">📖 {post.bookName}</p>}
        ...
        {activeTab === 'Reviews' && (
          <div className="space-y-4">
            {myReviews.length === 0 ? <div className="text-center py-8"><Star className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No reviews yet</p><button onClick={() => setPage('reviews')} className="mt-3 px-4 py-2 bg-orange-500 text-white rounded-lg text-sm">Write a Review</button></div>
        ...
        {activeTab === 'Books Read' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-gray-700">{myBooks.length} books read <Lock className="w-3 h-3 inline ml-1 text-gray-400" /></p>
              <button onClick={() => setShowAddBook(!showAddBook)} className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500 text-white rounded-lg text-sm font-medium"><Plus className="w-4 h-4" />{showAddBook ? 'Cancel' : 'Add Book'}</button>
            </div>
            {showAddBook && (
              <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                ...
                <div className="space-y-3">
                  <input value={newBook.title} onChange={e => setNewBook({...newBook, title: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Book title *" />
                  <input value={newBook.author} onChange={e => setNewBook({...newBook, author: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none" placeholder="Author" />
                  ...
                  <textarea value={newBook.notes} onChange={e => setNewBook({...newBook, notes: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none resize-none" placeholder="Notes (optional)" rows={2} />
            ...
            {myBooks.length === 0 ? <div className="text-center py-8"><BookMarked className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500 text-sm">No books tracked yet</p></div>
              : myBooks.map(book => (
        ...
        {activeTab === 'Saved Posts' && (
          <div className="space-y-4">
            <p className="text-xs text-gray-400 flex items-center gap-1 mb-2"><Lock className="w-3 h-3" />Only visible to you</p>
            {savedPostsList.length === 0 ? (
              <div className="text-center py-8"><Bookmark className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">No saved posts yet</p><p className="text-xs text-gray-400 mt-1">Save posts from the home feed to see them here!</p></div>
...
export default function App() {
  ...
  const [crews, setCrews] = useState([
    { id: 1, name: 'Atomic Habits', author: 'James Clear', genre: 'Self Improvement', policy: 'Weekly check-ins every Sunday', members: 1, chats: 0, createdBy: 'system', createdByName: 'ReadCrew', createdAt: new Date().toISOString() },
    { id: 2, name: 'Tuesdays with Morrie', author: 'Mitch Albom', genre: 'Inspiration', policy: 'Discuss one life lesson per session', members: 1, chats: 0, createdBy: 'system', createdByName: 'ReadCrew', createdAt: new Date().toISOString() },
    { id: 3, name: 'The Alchemist', author: 'Paulo Coelho', genre: 'Fiction', policy: 'Share personal legends and reflections', members: 1, chats: 0, createdBy: 'system', createdByName: 'ReadCrew', createdAt: new Date().toISOString() },
  ]);
  ...
  useEffect(() => {
    ...
    const rawCrews = JSON.parse(localStorage.getItem('crews') || '[]');
    if (rawCrews.length > 0) {
      const seen = new Set();
      const deduped = rawCrews.filter(c => {
        const key = `${(c.name||'').trim().toLowerCase()}__${(c.author||'').trim().toLowerCase()}`;
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      });
      if (deduped.length !== rawCrews.length) {
        localStorage.setItem('crews', JSON.stringify(deduped));
      }
      setCrews(deduped);
    }
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e) => { if (e.key?.includes('_notifications')) checkForNewNotifications(); };
  ...
  const handleLogin = (userData) => {
    ...
    localStorage.setItem('currentUser', JSON.stringify(userData));
    ['following','followers','blocked'].forEach(key => { if (!localStorage.getItem(`user_${userData.email}_${key}`)) localStorage.setItem(`user_${userData.email}_${key}`, JSON.stringify([])); });
  ...
  const handleReshare = (originalPost, comment) => {
    const allPosts = JSON.parse(localStorage.getItem('allPosts') || '[]');
    localStorage.setItem('allPosts', JSON.stringify(allPosts.map(p => p.id === originalPost.id ? { ...p, reshareCount: (p.reshareCount || 0) + 1 } : p)));
    if (originalPost.userEmail !== currentUser.email) {
      const notif = { id: Date.now(), type: 'reshare', fromUser: currentUser.name, fromUserEmail: currentUser.email, message: `${currentUser.name} reshared your post`, timestamp: new Date().toISOString(), read: false };
    ...
    handlePost({ id: Date.now(), content: originalPost.content, bookName: originalPost.bookName, author: originalPost.author, image: originalPost.image, isPublic: true, type: 'reshare', isReshare: true, originalPost: { id: originalPost.id, userName: originalPost.userName, userEmail: originalPost.userEmail, content: originalPost.content }, reshareComment: comment, reshareCount: 0 });
  };
  ...
  const handleFollow = (targetEmail, targetName) => {
    ...
    if (currentFollowing.includes(targetEmail)) {
      ...
      localStorage.setItem(`user_${targetEmail}_followers`, JSON.stringify(targetFollowers.filter(e => e !== currentUser.email)));
    } else {
      ...
      const notif = { id: Date.now(), type: 'follow', fromUser: currentUser.name, fromUserEmail: currentUser.email, message: `${currentUser.name} started following you`, timestamp: new Date().toISOString(), read: false };
    ...
    <div className="flex justify-center min-h-screen bg-gray-200">
      ...
      <div className="w-full max-w-md relative bg-white min-h-screen overflow-hidden shadow-xl">
        ...
        {showUserProfile && selectedUserProfile && (
          <UserProfileModal
            userEmail={selectedUserProfile.email} userName={selectedUserProfile.name}
            currentUser={currentUser} onClose={() => { setShowUserProfile(false); setSelectedUserProfile(null); }}
            onFollow={handleFollow} isFollowing={following.includes(selectedUserProfile.email)}
            onBlock={handleBlockUser} isBlocked={blockedUsers.includes(selectedUserProfile.email)}
        ...
        {viewingFullProfile && (
          <FullUserProfilePage
            viewedUserEmail={viewingFullProfile.email} viewedUserName={viewingFullProfile.name}
            currentUser={currentUser} onBack={() => setViewingFullProfile(null)}
            onFollow={handleFollow} isFollowing={following.includes(viewingFullProfile.email)}
            onBlock={handleBlockUser} isBlocked={blockedUsers.includes(viewingFullProfile.email)}
          />
        )}

        {!viewingFullProfile && (
          <>
            {currentPage === 'home' && <HomePage user={currentUser} posts={filteredPosts} setPosts={setPosts} crews={crews} setPage={setCurrentPage} donations={[]} reviews={[]} onUpdateStats={handleUpdateUser} updateNotificationCount={checkForNewNotifications} profileSrc={profileSrc} savedPosts={savedPosts} onSavePost={handleSavePost} onResharePost={handleReshare} onDeletePost={handleDeletePost} onFollow={handleFollow} following={following} onBlock={handleBlockUser} blockedUsers={blockedUsers} onViewUserProfile={handleViewUserProfile} />}
            ...
            {currentPage === 'explore' && <ExplorePage user={currentUser} setPage={setCurrentPage} onCreateCrew={(book) => {
              // Enforce uniqueness: skip if this book+author already has a crew
              ...
              if (existing) {
                alert(`A crew for "${book.title}" already exists! You can join it from the Crews page.`);
                setCurrentPage('crews');
                return;
              }
              const newCrew = { id: Date.now(), name: book.title.trim(), author: book.author.trim(), genre: book.genre || 'General', policy: '', members: 1, chats: 0, createdBy: currentUser.email, createdByName: currentUser.name, createdAt: new Date().toISOString() };
              const updatedCrews = [newCrew, ...crews];
              setCrews(updatedCrews);
              localStorage.setItem('crews', JSON.stringify(updatedCrews));
              setCurrentPage('crews');
            }} />}
            {currentPage === 'crews' && <CrewsPage user={currentUser} crews={crews} setPage={setCurrentPage} updateNotificationCount={checkForNewNotifications} onViewUserProfile={handleViewUserProfile} />}
            {currentPage === 'profile' && <ProfilePage user={currentUser} posts={filteredPosts} setPage={setCurrentPage} onLogout={handleLogout} onUpdateUser={handleUpdateUser} profileSrc={profileSrc} setProfileSrc={setProfileSrc} savedPosts={savedPosts} following={following} followers={followers} onFollow={handleFollow} />}
            {currentPage === 'notifications' && <NotificationsPage user={currentUser} onClose={() => { setCurrentPage('home'); checkForNewNotifications(); }} updateNotificationCount={checkForNewNotifications} />}
            <BottomNav active={currentPage} setPage={setCurrentPage} unreadCount={notificationCount} show={showBottomNav} />
          </>
        )}
      </div>
    </div>
